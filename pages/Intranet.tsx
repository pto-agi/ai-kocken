...
import { buildCompletionMapByDate, parseCompletedTaskIds } from '../utils/agendaCompletionState';
...
const Intranet: React.FC = () => {
  ...
  useEffect(() => {
    if (!intranetUserId || !canUseStaffIntranet) return;
    const days = getWorkweekDates(selectedDate);
    const dateKeys = days.map((day) => formatDateInput(day));
    const startKey = dateKeys[0];
    const endKey = dateKeys[dateKeys.length - 1];
    let active = true;
    const loadCompletion = async () => {
      let completionItemsAvailable = false;
      let completionItemRows: Array<{ report_date: string; task_id: string }> = [];
      let legacyRows: Array<{ report_date: string; completed_task_ids: unknown }> = [];

      try {
        if (isConfigured) {
          const { data, error } = await supabase
            .from('agenda_completion_items')
            .select('report_date,task_id')
            .eq('user_id', intranetUserId)
            .gte('report_date', startKey)
            .lte('report_date', endKey)
            .order('report_date', { ascending: true })
          ...
          completionItemRows = (data || []) as Array<{ report_date: string; task_id: string }>;
        }
      } catch (err) {
      ...
      try {
        if (isConfigured) {
          ...
          legacyRows = (data || []) as Array<{ report_date: string; completed_task_ids: unknown }>;
          ...
          setAgendaCompletionByDate((prev) => ({ ...prev, ...map }));
          return;
        }
      } catch (err) {
      ...
      if (!active) return;
      try {
        const localMap = dateKeys.reduce<Record<string, string[]>>((acc, dateKey) => {
          ...
          acc[dateKey] = Array.isArray(stored) ? stored.filter((value) => typeof value === 'string') : [];
          return acc;
        }, {});
        setAgendaCompletionByDate((prev) => ({ ...prev, ...localMap }));
      } catch {
        const emptyMap = dateKeys.reduce<Record<string, string[]>>((acc, dateKey) => {
          acc[dateKey] = [];
          return acc;
        }, {});
        setAgendaCompletionByDate((prev) => ({ ...prev, ...emptyMap }));
      }
    };
    loadCompletion();
    return () => {
