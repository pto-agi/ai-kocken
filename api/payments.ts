 ...
 * to their respective handlers imported from api/payments/.
 *
...
export default async function handler(req: any, res: any) {
  // Extract action from URL path: /api/payments/create-checkout-session → create-checkout-session
  const url = req.url || '';
  const match = url.match(/\/api\/payments\/([a-z\-]+)/);
  const action = match?.[1] || '';

  const routeHandler = routes[action];
  if (!routeHandler) {
    res.status(404).json({ error: `Unknown payment action: ${action || '(empty)'}` });
    return;
  }
